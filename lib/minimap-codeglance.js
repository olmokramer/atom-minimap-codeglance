...
var prototype = {
  ...
  addMinimap(minimap) {
    ...
    var minimapDisposable = new CompositeDisposable(
      atom.workspace.observeActivePaneItem(paneItem => {
        if(paneItem == minimap.getTextEditor()) {
      ...
      addEventListener(minimapView, 'mousedown', () =>
  ...
  setCurrentMinimap(minimap) {
    ...
    if(this.getTextEditor().getText() == this.element.getText()) return;
