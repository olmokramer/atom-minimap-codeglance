...
var prototype = {
  ...
  getTextEditor() {
    if(this.minimap) return this.minimap.getTextEditor();
  },
  ...
  scrollToMinimapOffset({offsetX, offsetY}) {
    if(this.disabled || !this.getTextEditor()) return;
  ...
  screenPositionForMinimapOffset(offsetX, offsetY) {
    ...
    var row = firstVisibleScreenRow + Math.floor(offsetY / this.getLineHeight());
    var column = Math.floor(offsetX / this.getColumnWidth());
    return new Point(row, column);
  },

  getLineHeight() {
    var {charHeight, interline} = this.minimap;
    if(!charHeight) charHeight = this.minimap.configCharHeight;
    if(!interline) interline = this.minimap.configInterline;
    return charHeight + interline;
  },

  getColumnWidth() {
    var {charWidth} = this.minimap;
    if(!charWidth) charWidth = this.minimap.configCharWidth;
    return charWidth;
  },
